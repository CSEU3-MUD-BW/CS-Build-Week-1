...
def initialize(request):
    ...
    return JsonResponse({'uuid': uuid, 'name': player.user.username, 'title': room.title, 'currentRoomID': room.id, 'description': room.description, 'players': players}, safe=True)


@csrf_exempt
@api_view(["GET"])
def rooms(request):
    """function to handle request whenever a user hits the rooms endpoint"""
    user = request.user
    if user:
        return JsonResponse(list(Room.objects.all().values("id", 'title', 'description', "n_to", "s_to", "e_to", "w_to")), safe=False, status=200)
    else:
        return JsonResponse({'error': "no user"}, safe=False, status=500)
...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        return JsonResponse({'name': player.user.username, 'title': nextRoom.title, 'description': nextRoom.description, 'players': players, 'nextRoomID': nextRoomID, 'error_msg': ""}, safe=True)
    else:
        players = room.playerNames(player_id)
